...
class JUnitBenchPressTestGenerator:
    def __init__(self, path="../Projeto/generatorTest/exercises/", filename="BenchPressGenTest.java"):
        self.path = path
    ...
    def gen_repeticoes(draw):
        return draw(st.integers(1, 50))

    @staticmethod
    @st.composite
    def gen_peso(draw):
        return draw(st.floats(10.0, 200.0))
...
class BenchPressGenTest {

    @BeforeEach 
    void setUp() { 
        new BenchPress().setProximoCodigo(1); 
    }
'''
    ...
    void {nome}() {{
        ...
        assertNotNull(bp);
        assertNotNull(bp.getDataRealizacao());
        assertNotNull(bp.getTempo());
        assertEquals(0, bp.getFreqCardiaca());
        assertEquals(0, bp.getRepeticoes());
        assertEquals(0.0, bp.getPeso(), 0.01);
        assertTrue(bp.getCodAtividade() > 0);
    }}'''
    ...
    void {nome}() {{
        ...
        int r = {args[3]};
        double p = {args[4]:.2f};
        BenchPress bp = new BenchPress(d, t, f, r, p);
        assertNotNull(bp);
        assertEquals(d, bp.getDataRealizacao());
        assertEquals(t, bp.getTempo());
        assertEquals(f, bp.getFreqCardiaca());
        assertEquals(r, bp.getRepeticoes());
        assertEquals(p, bp.getPeso(), 0.01);
        assertTrue(bp.getCodAtividade() > 0);
    }}'''
    ...
    void {nome}() {{
        ...
        int r = {args[3]};
        double p = {args[4]:.2f};
        BenchPress original = new BenchPress(d, t, f, r, p);
        BenchPress copia = new BenchPress(original);
        assertNotNull(copia);
    ...
    void {nome}() {{
        ...
        BenchPress bp1 = new BenchPress(d1, t1, {f1}, {r1}, {p1:.2f});
        BenchPress bp2 = new BenchPress(d2, t2, {f2}, {r2}, {p2:.2f});
        {cmp}(bp1.equals(bp2));
    }}'''
    ...
    void {nome}() {{
        ...
        int r = {args[3]};
        double p = {args[4]:.2f};
        BenchPress atividade = new BenchPress(d, t, f, r, p);
        ...
        double fatorReps = atividade.getFatorRepeticoes(0.25, 0.2);
        double fatorPeso = atividade.getFatorPeso(utilizador, 0.5, 0.2);
    ...
    void {nome}() {{
        ...
        assertInstanceOf(atividade instanceof BenchPress);
        assertTrue(atividade.getTempo().toSecondOfDay() > 0);
        assertEquals(0, atividade.getFreqCardiaca());
        
        BenchPress bpGen = (BenchPress) atividade;
        assertTrue(bpGen.getRepeticoes() >= 0);
        assertTrue(bpGen.getPeso() > 0);
        
        // Verificar se o peso gerado é razoável (deve ser o peso do utilizador se não há histórico)
        assertEquals(utilizador.getPeso(), bpGen.getPeso(), 0.01);
    }}'''
    ...
    void {nome}() {{
        ...
        assertTrue(resultado.contains("Bench press"), "ToString deve conter 'Bench press'");
        assertTrue(resultado.length() > 0, "ToString não deve estar vazio");
    }}'''

        elif tipo == "SettersGetters":
            return f'''
    @Test
    void {nome}() {{
        BenchPress bp = new BenchPress();
        double novoPeso = {args[0]:.2f};
        int novasReps = {args[1]};
        
        bp.setPeso(novoPeso);
        bp.setRepeticoes(novasReps);
        
        assertEquals(novoPeso, bp.getPeso(), 0.01);
        assertEquals(novasReps, bp.getRepeticoes());
    }}'''
    ...
    @given(gen_peso(), gen_repeticoes())
    def _test_setters_getters(self, peso, reps):
        metodo = self.gerar_metodo_teste("SettersGetters", peso, reps)
        self.adicionar_metodo(metodo)

if __name__ == "__main__":
    gen = JUnitBenchPressTestGenerator()
