...
class JUnitFlexoesTestGenerator:
    def __init__(self, path="../Projeto/generatorTest/exercises/", filename="FlexoesGenTest.java"):
        self.path = path
...
class FlexoesGenTest {

    @BeforeEach 
    void setUp() { 
        new Flexoes().setProximoCodigo(1); 
    }
'''
    ...
    void {nome}() {{
        ...
        assertNotNull(f);
        assertNotNull(f.getDataRealizacao());
        assertNotNull(f.getTempo());
        assertEquals(0, f.getFreqCardiaca());
        assertEquals(0, f.getRepeticoes());
        assertTrue(f.getCodAtividade() > 0);
    }}'''
    ...
    void {nome}() {{
        ...
        int freq = {args[2]};
        int reps = {args[3]};
        Flexoes f = new Flexoes(d, t, freq, reps);
        assertNotNull(f);
        assertEquals(d, f.getDataRealizacao());
        assertEquals(t, f.getTempo());
        assertEquals(freq, f.getFreqCardiaca());
        assertEquals(reps, f.getRepeticoes());
        assertTrue(f.getCodAtividade() > 0);
    }}'''
    ...
    void {nome}() {{
        ...
        int freq = {args[2]};
        int reps = {args[3]};
        Flexoes original = new Flexoes(d, t, freq, reps);
    ...
    void {nome}() {{
        ...
        Flexoes f1 = new Flexoes(d1, t1, {f1}, {r1});
        Flexoes f2 = new Flexoes(d2, t2, {f2}, {r2});
        {cmp}(f1.equals(f2));
    }}'''
    ...
    void {nome}() {{
        ...
        int freq = {args[2]};
        int reps = {args[3]};
        Flexoes atividade = new Flexoes(d, t, freq, reps);
        ...
        double expected = 3.5 * (utilizador.getFatorMultiplicativo() + fatorReps + fatorFreq) * utilizador.getBMR() / (24 * 60 * 60) * segundos;
    ...
    @Test
    void {nome}() {{
        Flexoes flexoes = new Flexoes();
        Utilizador utilizador = new UtilizadorAmador();
        double consumoCalorias = {args[0]:.2f};
        
        Atividade atividade = flexoes.geraAtividade(utilizador, consumoCalorias);
        ...
        Flexoes flexoesGen = (Flexoes) atividade;
        assertTrue(flexoesGen.getRepeticoes() > 0);
        
        // Verificar se as repetições são calculadas corretamente (tempo * 1)
        int tempoSegundos = atividade.getTempo().toSecondOfDay();
        assertEquals(tempoSegundos, flexoesGen.getRepeticoes());
    }}'''
    ...
    void {nome}() {{
        ...
        assertTrue(resultado.contains("Flexões"), "ToString deve conter 'Flexões'");
