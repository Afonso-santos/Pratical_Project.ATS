...
class JUnitCiclismoTestGenerator:
    def __init__(self, path="../Projeto/generatorTest/exercises/", filename="CiclismoGenTest.java"):
        self.path = path
...
class CiclismoGenTest {

    @BeforeEach 
    void setUp() { 
        new Ciclismo().setProximoCodigo(1); 
    }
'''
    ...
    void {nome}() {{
        ...
        Ciclismo c = new Ciclismo(d, t, f, dist);
        assertNotNull(c);
    ...
    void {nome}() {{
        ...
        int f = {args[2]};
        double dist = {args[3]:.2f};
        Ciclismo original = new Ciclismo(d, t, f, dist);
        Ciclismo copia = new Ciclismo(original);
        assertNotNull(copia);
    ...
    void {nome}() {{
        ...
        Ciclismo c1 = new Ciclismo(d1, t1, {f1}, {dist1:.2f});
        Ciclismo c2 = new Ciclismo(d2, t2, {f2}, {dist2:.2f});
        {cmp}(c1.equals(c2));
    }}'''
    ...
    void {nome}() {{
        ...
        int f = {args[2]};
        double dist = {args[3]:.2f};
        Ciclismo atividade = new Ciclismo(d, t, f, dist);
    ...
    void {nome}() {{
        ...
        Atividade atividade = ciclismo.geraAtividade(utilizador, consumoCalorias);
        ...
        Ciclismo ciclismoGen = (Ciclismo) atividade;
        assertTrue(ciclismoGen.getDistancia() > 0);
        
        // Verificar se a distância foi calculada corretamente (tempo * 10.5)
        double tempoSegundos = atividade.getTempo().toSecondOfDay();
        double distanciaEsperada = tempoSegundos * 10.5;
        assertEquals(distanciaEsperada, ciclismoGen.getDistancia(), 0.01);
    }}'''
    ...
    void {nome}() {{
        ...
        assertTrue(resultado.contains("Ciclismo"), "ToString deve conter 'Ciclismo'");
...
if __name__ == "__main__":
    gen = JUnitCiclismoTestGenerator()
